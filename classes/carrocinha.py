...
class Carrocinha:
    ...
    def carrocinha(self, relogio, personagem):
        ...
        if escolha == 1:  # Escolha Fugir
            # Sorteia um número para validação
            sorteio_carrocinha = randint(1, 2)
            if sorteio_carrocinha == 1:  # Cao conseguiu fugir
                ...
                print(
                    f'{personagem.nome} não conseguiu fugir, você pode tentar novamente, lembrando que se sua energia chegar a 0, GAME OVER!')
                # Random para decidir se o cão foi doado
                adocao = randint(1, 3)
                while True:
                    print(
                        f"{personagem.nome} tentou fugir com todas suas forças mas falhou!")
                    teste = str(
            ...
            if sorteio_carrocinha == 1:  # Conseguiu se esconder
                ...
                while True:
                    print(
                        f"{personagem.nome} tentou se esconder em todos os lugares, mas falhou!")
                    teste = str(
                    ...
                    if teste == "S":
                        ...
                        print(
                            "Voce tentou, tentou, tentou se esconder mas não conseguiu, desistiu e foi levado pela carrocinha")
                        personagem.muda_lugar('carrocinha')
                        # Se o dia for igual a 1 o cachorro nao tem a opção de ser adotado (apenas a partir do dia 2)
                        if relogio.dia == 1:
                            personagem.muda_fome(-10)
