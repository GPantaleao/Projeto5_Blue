    ...
    def carrocinha(self, relogio, personagem):
        ...
        if escolha == 1: #Escolha Fugir
            ...
            if sorteio_carrocinha == 1: #Cao conseguiu fugir
                ...
                print(f'{personagem.nome} corre apressado entre as ruas, fugindo do carro da carrocinha e daquele homem uniformizado que insiste em tentar laçar seu pescoço. Entre os becos encontra um lugar seguro atrás de umas caixas jogadas, depois dessa longa perseguição se sente exausto e assustado.')
                ...
                print(
                    f'{personagem.nome} corre desesperadamente para se safar, passando correndo entre as pessoas que somente observam a cena, ele(a) implora ajuda, entretanto ninguém faz nada, somente observa a situação. Ele(a) corre para uma rua estreita na esperança de se esconder, entretanto não há saída e nosso amigo(a) é capturado pelo homem uniformizado')
                # Random para decidir se o cão foi doado
                adocao = randint(1, 5)
                while True:
                    print(
                        f"{personagem.nome} se debate desesperamente em vão. Entretanto agora você tem a chance de ajudar nosso amigo(a)!!!")
                    teste = str(
                    ...
                    if teste == "S":
                        ...
                        if pepate.jogadaCao == True:
                            relogio.avanca_tempo(80)
                            print(f'\nDeu certo!!! Você conseguiu criar uma distração e ajudar {personagem.nome}. E agora nosso(a) amigo(a) foge o mais rápido do que pode. Ele(a) precisa se esconder e se acalmar depois desse susto')
                            personagem.muda_fome(-10)
                        ...
                        print(f"\nNem sempre com agilidade e malandragem das ruas é fácil conseguir fugir,apesa de muita luta, {personagem.nome} é levado para uma gaiola apertada da carrocinha, cheio de medo chora assustado")
            ...
            if sorteio_carrocinha == 1:#Conseguiu se esconder
                relogio.avanca_tempo(80)
                print(f'\n{personagem.nome} corre sem rumo entra as ruas, sem se atrever a olhar para tras, corre tanto que suas pernas estavam ja cansadas e tremendo, desesperado procura um lugar seguro para que possa se esconder até a situação acalmar, entre os becos encontra uma casa abandonada com uma fresta quebrada na porta, e com esforço, rastejando consegue entrar e ficar seguro até que a poiera baixe.')
                personagem.muda_fome(10)
                personagem.muda_energia(10)
                personagem.muda_humor(5)
            elif sorteio_carrocinha == 2: #Não conseguiu se esconder
                print(f'{personagem.nome} corre assustado entre os carros, desesperado não tem ideia onde esta e como pode se esconder. Mas logo atras esta os homens uniformizados o(a) perseguindo novamente e conseguem alcança-lo(a) antes de conseguir se esconder...\n''')
                sleep(1)
                adocao = randint(1,4)
                while True:
                    ...
                    if teste == "S": 
                        ...
                        if dado.caoDado == True:
                            relogio.avanca_tempo(80)
                            print(f'\nApós a sua distração, {personagem.nome} consegue fugir apressadamente entre os carros e depistar os homens uniformizados e ainda muito assustado(a) busca refugiu entre os carros de um pátio abandonado.\n')
                            personagem.muda_fome(-10)
                            personagem.muda_energia(-15)
                            personagem.muda_humor(-5)
                            break
                    else:
                        print(f'\nApesar de tudo {personagem.nome} não consegue fugir da perseguição dos homens uniformizados e novamente é preso na gaiola fria da carrocinha')
                        personagem.muda_lugar('carrocinha')
                        if relogio.dia == 1: #Se o dia for igual a 1 o cachorro nao tem a opção de ser adotado (apenas a partir do dia 2)
                            personagem.muda_fome(-10)
                            personagem.muda_energia(-10)
